...
/**
 * /dashboard/mfa — Admin TOTP enrollment / disable.
 * Mirrors the portal's /dashboard/settings/mfa page. Admin accounts
 * SHOULD have MFA enrolled — this is the one place they can do it
...
export default function AdminMfaPage() {
  ...
  async function startEnrollment() {
    ...
    try {
      ...
      const { data, error: enrollErr } = await supabase.auth.mfa.enroll({
        factorType: 'totp',
        issuer: 'PRECISO Admin',
        friendlyName: 'PRECISO Admin Authenticator',
      });
      if (enrollErr || !data) {
      ...
      console.error('[admin/mfa/enroll] unexpected error', err);
  ...
  async function confirmEnrollment(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.error('[admin/mfa/verify] unexpected error', err);
      setError('Unexpected error. Please try again.');
    } finally {
      setPending(false);
    }
  ...
  async function disableMfa() {
    ...
    const confirmed = window.confirm(
      'Disabling MFA lowers the security of this admin account. Continue?',
    );
    if (!confirmed) return;
    setError(null);
    setNotice(null);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mb-6 text-sm text-gray-500">
        MFA is strongly recommended for every admin account. All sign-in
      ...
      {loading ? (
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mb-6 flex flex-col items-center gap-4 rounded-lg border border-gray-200 bg-gray-50 p-6">
            ...
            <p className="text-xs text-gray-500">Can&apos;t scan? Enter this secret manually:</p>
          ...
          <form onSubmit={confirmEnrollment}>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="rounded-lg bg-navy px-5 py-2.5 text-sm font-medium text-white transition hover:bg-navy-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <p className="mb-6 text-sm text-gray-600">
            Admin accounts should always have MFA. Enroll an authenticator app
            now to protect the platform-wide console.
          </p>
          <button
            type="button"
            onClick={startEnrollment}
            disabled={pending}
            className="rounded-lg bg-navy px-5 py-2.5 text-sm font-medium text-white transition hover:bg-navy-600 disabled:opacity-50"
          >
