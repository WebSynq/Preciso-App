...
/**
 * POST /api/auth/mfa/verify (developer console)
 *
 * Mirrors the portal + admin MFA verify routes. Namespaced under 'dev:'
 * lockout keys and 'auth.dev_mfa_*' action names. Developer role claim
 * re-checked on success.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const emailKey = `login-failed:dev:email:${user.email?.toLowerCase() ?? user.id}`;
    ...
    console.error('[dev/auth/mfa/verify] unexpected error', { requestId, err });
    return NextResponse.json(
...
async function writeAuditLog(opts: {
  ...
  try {
    ...
    console.error('[dev/auth/mfa/verify] audit insert failed', { requestId: opts.requestId, err });
  }
}
